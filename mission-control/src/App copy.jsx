...
async function fetchSpotifyData(token){
  const spotify = await fetch("https://api.spotify.com/v1/me/player/recently-played", {
    method:'get',
    headers: new Headers({
...
async function fetchAudioFeatures(token, recentIDs){
...
class Spotify extends React.Component {
  ...
  getAudioFeatures(token){
    fetchAudioFeatures(token).then(data=>{
 ...
 componentDidMount() {
  ...
  this.getSpotifyData()
  setInterval(this.getSpotifyData,10000)
  this.getAudioFeatures()
  setInterval(this.getAudioFeatures,10000)
  ...
  render() {
    const { logout, token, recentTracks, recentIDs, danceability } = this.state;
    // console.log(token);
    return (
      <div>
        {!token ?
